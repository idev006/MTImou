...
pub async fn p2p_handshake(
    ...
    */
    ...
    socket2.connect(agent).await.unwrap();
    let relay_chan = time::timeout(time::Duration::from_secs(8), socket2.dh_read()).await;
    if relay_chan.is_err() {
        println!("Warning: relay-channel response timeout; continuing anyway.");
    }

    let mut session = PTCPSession::new();

    socket2.ptcp_request(session.send(PTCPBody::Sync)).await;
    let sync_packet = time::timeout(time::Duration::from_secs(8), socket2.ptcp_read()).await;
    if let Ok(packet) = sync_packet {
        session.recv(packet);
    } else if relay_mode {
        println!("Warning: no PTCP sync response in relay mode; continuing anyway.");
        return (socket2, session);
    } else {
        panic!("Timeout waiting for PTCP sync");
    }
